 ...
 */
public class CustomRecycleView extends RecyclerView implements View.OnFocusChangeListener{
    ...
    public void smoothToCenter(int position){
        if(mTargetChild!=null){//先把之前的动画还原
            showUnSelectAnimation(mTargetChild);
        }
        ...
        mTargetChild = getChildAt(mTargetPos-firstvisiableposition);//获取目标item在当前可见视图item集合中的位置
        ...
        int childLeftPx = mTargetChild.getLeft();//子view相对于父view的左边距
        int childRightPx = mTargetChild.getRight();//子view相对于父view的右边距
        Log.i(TAG, "target-->left:" + mTargetChild.getLeft() + "   right:" + mTargetChild.getRight());


        int childWidth = mTargetChild.getWidth();
        ...
        showSelectedAnimation(mTargetChild);//加入缩放动画
    }

    private void showSelectedAnimation(View view){
    ...
    private void showUnSelectAnimation(View view){
        ScaleAnimation animation = new ScaleAnimation(1.2f,1.0f,1.2f,1.0f, Animation.RELATIVE_TO_SELF,
                0.5f,Animation.RELATIVE_TO_SELF,0.5f);
        animation.setDuration(200);
        animation.setFillAfter(true);
        view.startAnimation(animation);
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        if (hasFocus) {
            if (getChildCount() > 0) {
                getChildAt(0).requestFocus();//必须在recycleview获得焦点后才可以设置
            }
        }
    }
}
